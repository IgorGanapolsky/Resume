...
for row in rows:
    if (
        ...
        row["Notes"] = (
            "Retrying with Maintainer-focused PDF resume via Anchor Browser CI."
        )
    elif "Anyscale" in row["Company"] and "AI / ML Solutions Engineer" in row["Role"]:
        row["Status"] = "ReadyToSubmit"
        row["Submitted Resume Path"] = (
            "applications/anthropic/tailored_resumes/Igor_Ganapolsky_Open_Source_Maintainer_AI.pdf"  # Using the same PDF for now to test
        )
