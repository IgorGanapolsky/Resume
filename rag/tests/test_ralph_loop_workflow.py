...
def test_live_submit_requires_profile_and_answers_but_not_auth():
    ...
    assert "execute_submissions:" in text  # nosec B101
    assert "--execute-submissions" in text  # nosec B101
    assert (
        "if ! python3 scripts/ci_submit_pipeline.py \\" in text
        and "--validate-secrets-only \\" in text
    )
    assert "CI submit secrets invalid; falling back to dry run." in text  # nosec B101
    assert (
